  ...
  const handleForge = useCallback(async (prompt: string, settings: ForgeSettings) => {
    ...
    try {
      ...
      if (err.message.includes("fetch") || err.message.includes("Failed")) {
        setError("⚠️ Backend not connected. Running in DEMO MODE.");
        const demoResult: ForgeResult = {
          ...
          originalResponse: "Demo response for original prompt.",
          optimizedResponse: "Demo response for optimized prompt. Connect backend for real results!",
          originalScore: 3.5,
          optimizedScore: 8.0,
        };
        setPipelineStep(6);
